...
class UniswapWrapper:
    def __init__(self, address, private_key, provider=None, network="mainnet"):
        ...
        path = "./assets/"
        with open(os.path.abspath(path + "uniswap_addr.json")) as f:
            token_and_exchange_addresses = json.load(f)['tokens']
        ...
        self.token_address_from_symbol = {}
        self.token_address_from_exchange = {}
        self.exchange_contract = {}
        self.erc20_contract = {}

        for item in token_and_exchange_addresses.items():
            symbol = item[0]
            token_address = item[1]['tokenAddress']
            exchange_address =  Web3.toChecksumAddress( item[1]['address'] )
            self.exchange_address_from_token[token_address] = exchange_address
            self.token_address_from_symbol[symbol] = token_address
